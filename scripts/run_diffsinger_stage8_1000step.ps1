...
Write-Host '[Phoenix] Stage8-1000: validating binary Dataset integrity...' -ForegroundColor Cyan
& $python $preflight --diffsinger $DiffSinger --config $Config --binary $Binary
if ($LASTEXITCODE -ne 0) { throw 'Stage8-1000 binary preflight failed.' }
...
try {
    ...
    $expName = 'phoenix_freed_joud_stage8_1000step'
    Write-Host '[Phoenix] Stage8-1000: starting controlled 1000-step training...' -ForegroundColor Cyan
    & $python $train `
        --config $Config `
        --exp_name $expName `
        --reset `
        --hparams "max_updates=1000,max_batch_frames=8000,max_batch_size=1,max_val_batch_frames=8000,max_val_batch_size=1,val_check_interval=25,num_sanity_val_steps=1,log_interval=10,ds_workers=1,dataloader_prefetch_factor=2,num_ckpt_keep=8,permanent_ckpt_start=999999,permanent_ckpt_interval=999999,val_with_vocoder=False"
    if ($LASTEXITCODE -ne 0) { throw 'Stage8-1000 training failed.' }
}
finally {
    Set-Location $oldPwd
...
$work = Join-Path $DiffSinger "checkpoints\phoenix_freed_joud_stage8_1000step"
if (-not (Test-Path $work)) { throw "Training completed but checkpoint work directory was not found: $work" }

Write-Host "[Phoenix] Stage8-1000 completed. Experiment: $work" -ForegroundColor Green
