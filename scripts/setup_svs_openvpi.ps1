...
Write-Host '[Phoenix] Installing CUDA-enabled PyTorch 2.6.0 for SVS...' -ForegroundColor Cyan
& $envPip install --index-url https://download.pytorch.org/whl/cu126 `
...
& $envPython -c "import sys; print('Python:',sys.version); import torch; print('Torch:',torch.__version__); print('CUDA:',torch.cuda.is_available()); print('Runtime:',torch.version.cuda); print('GPU:',torch.cuda.get_device_name(0) if torch.cuda.is_available() else 'NONE'); print('VRAM_GB:',round(torch.cuda.get_device_properties(0).total_memory/1024**3,2) if torch.cuda.is_available() else 0)"
