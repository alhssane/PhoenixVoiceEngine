...
Write-Host '[Phoenix] Stage7-50: validating binary Dataset integrity...' -ForegroundColor Cyan
& $python $preflight --diffsinger $DiffSinger --config $Config --binary $Binary
if ($LASTEXITCODE -ne 0) { throw 'Stage7-50 binary preflight failed.' }
...
try {
    ...
    $expName = 'phoenix_freed_joud_smoke_50step'
    Write-Host '[Phoenix] Stage7-50: starting 50-step DiffSinger training smoke...' -ForegroundColor Cyan
    & $python $train `
        --config $Config `
        --exp_name $expName `
        --reset `
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Stage7-50 training failed.' }
}
finally {
    Set-Location $oldPwd
...
$work = Join-Path $DiffSinger "checkpoints\phoenix_freed_joud_smoke_50step"
if (-not (Test-Path $work)) { throw "50-step training completed but checkpoint work directory was not found: $work" }

Write-Host "[Phoenix] Stage7-50 completed. Experiment: $work" -ForegroundColor Green
