param(
    ...
    [Parameter(Mandatory=$false)][string]$ExpName = 'phoenix_freed_joud_fixed_v2_smoke_2step'
)
...
Write-Host "[Phoenix] Stage7-V2: validating canonical binary dataset..." -ForegroundColor Cyan
& $python $preflight --diffsinger $DiffSinger --config $Config --binary $Binary
if ($LASTEXITCODE -ne 0) { throw 'Stage7-V2 binary preflight failed.' }
...
try {
    ...
    Write-Host "[Phoenix] Stage7-V2: starting two-step smoke on $ExpName..." -ForegroundColor Cyan
    & $python $train `
        --config $Config `
        --exp_name $ExpName `
        --reset `
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Stage7-V2 two-step training smoke failed.' }
}
finally {
    Set-Location $oldPwd
...
Write-Host "[Phoenix] Stage7-V2 completed. Smoke experiment: $work" -ForegroundColor Green
