...
def fit_token_sequence_to_frames(pairs: list[tuple[int, int]], token_count: int, hop_s: float, audio_duration: float) -> list[dict[str, float | int | bool]]:
    ...
    for pos, idx in enumerate(valid_indices):
        ...
        end = max(center, last_center) if pos == len(valid_indices) - 1 else (center + valid_centers[pos + 1]) * 0.5
...
def build(stage1: Path, stage2: Path, output: Path) -> dict:
    ...
    report = {
        ...
        "note": "Arabic IPA is mapped to the model's published phone symbols. Token durations are contiguous boundaries between token centers, including blank frames.",
    }
