...
def load_one_segment(path: Path) -> dict[str, Any]:
    ...
    # Phoenix historical DS files are commonly a single JSON object, while
    # OpenVPI inference also accepts a one-element JSON list. Support both.
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Phoenix DiffSinger acoustic text-causality audit v2.")
    ...
    try:
        os.chdir(root)
        sys.argv = [
            "phoenix_acoustic_text_causality_audit_v2.py",
            "--exp_name",
            args.exp,
            "--infer",
        ]
        ...
        result = {
            "status": "PHOENIX_ACOUSTIC_TEXT_CAUSALITY_AUDIT_V2",
            "exp": args.exp,
            "ckpt": args.ckpt,
