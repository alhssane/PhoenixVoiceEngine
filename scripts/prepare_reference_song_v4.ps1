...
if ($args.Count -lt 1) {
    throw 'Usage: prepare_reference_song_v4.ps1 <AudioPath> [ProjectName] [ArtistName]'
}
...
Write-Host '[Phoenix] Installing Phoenix runtime dependencies...' -ForegroundColor Cyan
& $python -m pip install -r $requirements
if ($LASTEXITCODE -ne 0) { throw 'Phoenix runtime dependency installation failed.' }

Write-Host '[Phoenix] Ensuring core audio dependencies...' -ForegroundColor Cyan
...
$cacheRoot = Join-Path $projectRoot '.cache\foundation_sync_v4'
$zipPath = Join-Path $projectRoot '.cache\foundation-hardening-v4.zip'
...
Get-ChildItem $sourceSrc -Recurse -File | ForEach-Object {
    $relative = $_.FullName.Substring($sourceSrc.Length).TrimStart([char[]]@('\','/'))
