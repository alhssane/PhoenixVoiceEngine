param(
    ...
    [Parameter(Mandatory=$false)][string]$ExpName = 'phoenix_freed_joud_fixed_v2_smoke_500step'
)
...
Write-Host "[Phoenix] Stage7-V2-500: validating canonical binary dataset..." -ForegroundColor Cyan
& $python $preflight --diffsinger $DiffSinger --config $Config --binary $Binary
if ($LASTEXITCODE -ne 0) { throw 'Stage7-V2-500 binary preflight failed.' }
...
try {
    ...
    Write-Host "[Phoenix] Stage7-V2-500: starting 500-step training on $ExpName..." -ForegroundColor Cyan
    & $python $train `
        --config $Config `
        --exp_name $ExpName `
        --reset `
        --hparams "max_updates=500,max_batch_frames=8000,max_batch_size=1,max_val_batch_frames=8000,max_val_batch_size=1,val_check_interval=25,num_sanity_val_steps=1,log_interval=10,ds_workers=1,dataloader_prefetch_factor=2,permanent_ckpt_start=999999,permanent_ckpt_interval=999999,val_with_vocoder=False"
    if ($LASTEXITCODE -ne 0) { throw 'Stage7-V2-500 training failed.' }
}
finally {
    Set-Location $oldPwd
...
if (-not (Test-Path $work)) { throw "500-step training completed but checkpoint work directory was not found: $work" }

Write-Host "[Phoenix] Stage7-V2-500 completed. Experiment: $work" -ForegroundColor Green
