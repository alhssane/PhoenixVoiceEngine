...
class SongProjectEngine:
    """End-to-end coordinator for clean-vocal Arabic singing edits."""
    ...
    def prepare(self, audio_path: str | Path, project_name: str, artist_name: str) -> dict[str, Any]:
        ...
        manifest = {
            ...
            "language": "ar",
            "synthesis_architecture": "hybrid_svs_voice_conversion",
        }
    ...
    @staticmethod
    def _backend():
        if os.getenv("PHOENIX_SVS_COMMAND", "").strip() and os.getenv("PHOENIX_VC_COMMAND", "").strip():
            return HybridSingingBackend()
        return CommandSynthesisBackend()
    ...
    def synthesize_edit(
        ...
        reference_lyrics = " ".join(str(item["word"]) for item in words if item.get("word"))
        ...
        if not backend.supports("ar"):
            raise RuntimeError(
                "Arabic singing backend is not configured. Set both "
                "PHOENIX_SVS_COMMAND and PHOENIX_VC_COMMAND for the hybrid "
                "pipeline, or PHOENIX_SYNTH_COMMAND for a single backend."
            )
