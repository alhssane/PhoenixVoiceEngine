...
IPA_CHAR_MAP = {
    "ʔ": "<",
    "ء": "<",  # Epitran may emit literal Arabic hamza at word-final position.
    "b": "b", "t": "t", "d": "d", "k": "k", "q": "q",
    "f": "f", "s": "s", "z": "z",
    "ʃ": "sh", "ʒ": "j",
    "x": "x", "χ": "x", "ɣ": "g", "ɢ": "g",
...
def normalize_arabic_for_phonemization(text: str) -> str:
    ...
    text = text.replace("ـ", "")
    # Orthographic forms that Epitran may leave literal. For singing lyrics,
...
def ipa_to_canonical(ipa: str, *, word: str = "") -> list[str]:
    ...
    if invalid:
        raise UnsupportedArabicPhone(f"Invalid canonical phones {invalid!r}: word={word!r}")
    return tokens
